...
const dbpf_1 = __webpack_require__(/*! ../src/dbpf */ "../src/dbpf");
const input = document.getElementById('input');
const files = globalThis.files = [];
const dbpfs = globalThis.dbpfs = [];
...
input?.addEventListener("change", function (event) {
    ...
    if (event.target.files.length) {
        const these_files = event.target.files;
        files.push(...these_files);
        for (let file of these_files) {
            const dbpf = new dbpf_1.DBPF(file);
            dbpfs.push(dbpf);
            const dbpfElement = templates.file?.cloneNode(true);
            ...
            [
                ...
                "hole-offset",
                "dbpf-contents"
            ].forEach((className) => dbpfChildren[className] = dbpfElement.querySelector(`.${className}`));
            ...
            const magicText = `0x${magic.toString(16).padStart(8, "0").toUpperCase()} (=${magic})`;
            dbpfChildren["magic"].textContent = magicText;
            output?.appendChild(dbpfElement);
            dbpf.init().then((initialized) => {
                const dbpfMajor = initialized.header.dbpf.major;
                const dbpfMajorText = `0x${dbpfMajor.toString(16).padStart(8, "0").toUpperCase()} (=${dbpfMajor})`;
                const dbpfMinor = initialized.header.dbpf.minor;
                const dbpfMinorText = `0x${dbpfMinor.toString(16).padStart(8, "0").toUpperCase()} (=${dbpfMinor})`;
                const dbpfUserMajor = initialized.header.dbpf.usermajor;
                const dbpfUserMajorText = `0x${dbpfUserMajor.toString(16).padStart(8, "0").toUpperCase()} (=${dbpfUserMajor})`;
                const dbpfUserMinor = initialized.header.dbpf.userminor;
                const dbpfUserMinorText = `0x${dbpfUserMinor.toString(16).padStart(8, "0").toUpperCase()} (=${dbpfUserMinor})`;
                const dbpfCreated = initialized.header.dbpf.created;
                const dbpfCreatedText = `0x${dbpfCreated.toString(16).padStart(8, "0").toUpperCase()} (=${dbpfCreated}) (${new Date(dbpfCreated).toLocaleString()})`;
                const dbpfModified = initialized.header.dbpf.modified;
                const dbpfModifiedText = `0x${dbpfModified.toString(16).padStart(8, "0").toUpperCase()} (=${dbpfModified}) (${new Date(dbpfModified).toLocaleString()})`;
                ...
                const indexMajorText = `0x${indexMajor.toString(16).padStart(8, "0").toUpperCase()} (=${indexMajor})`;
                const indexMinor = initialized.header.index.minor;
                const indexMinorText = `0x${indexMinor.toString(16).padStart(8, "0").toUpperCase()} (=${indexMinor})`;
                const indexCount = initialized.header.index.count;
                const indexCountText = `0x${indexCount.toString(16).padStart(8, "0").toUpperCase()} (=${indexCount})`;
                const indexSize = initialized.header.index.size;
                const indexSizeText = `0x${indexSize.toString(16).padStart(8, "0").toUpperCase()} (=${indexSize})`;
                const indexOffset = initialized.header.index.offset;
                const indexOffsetText = `0x${indexOffset.toString(16).padStart(8, "0").toUpperCase()} (=${indexOffset})`;
                const indexFirst = initialized.header.index.first;
                const indexFirstText = `0x${indexFirst.toString(16).padStart(8, "0").toUpperCase()} (=${indexFirst})`;
                ...
                const holeCountText = `0x${holeCount.toString(16).padStart(8, "0").toUpperCase()} (=${holeCount})`;
                const holeSize = initialized.header.hole.size;
                const holeSizeText = `0x${holeSize.toString(16).padStart(8, "0").toUpperCase()} (=${holeSize})`;
                const holeOffset = initialized.header.hole.offset;
                const holeOffsetText = `0x${holeOffset.toString(16).padStart(8, "0").toUpperCase()} (=${holeOffset})`;
                ...
                for (let index of indeces) {
                    ...
                    entries.push(entry);
                    const entryElement = templates.contents?.cloneNode(true);
                    ...
                    [
                        ...
                        "resource-size-file",
                    ].forEach((className) => entryChildren[className] = entryElement.querySelector(`.${className}`));
                    entryChildren["resource-index"].textContent = index;
                    const group = entry.group;
                    const groupText = `0x${group.toString(16).padStart(8, "0").toUpperCase()} (=${group})`;
                    const type = entry.type;
                    const typeText = `0x${type.toString(16).padStart(8, "0").toUpperCase()} (=${type})`;
                    const instance = entry.instance;
                    const instanceText = `0x${instance.toString(16).padStart(16, "0").toUpperCase()} (=${instance})`;
                    const offset = entry.offset;
                    const offsetText = `0x${offset.toString(16).padStart(8, "0").toUpperCase()} (=${offset})`;
                    const sizeCompressed = entry.size.compressed;
                    const sizeCompressedText = `0x${sizeCompressed.toString(16).padStart(8, "0").toUpperCase()} (=${sizeCompressed})`;
                    const sizeMemory = entry.size.memory;
                    const sizeMemoryText = `0x${sizeMemory.toString(16).padStart(8, "0").toUpperCase()} (=${sizeMemory})`;
                    const sizeFile = entry.size.file;
                    const sizeFileText = `0x${sizeFile.toString(16).padStart(8, "0").toUpperCase()} (=${sizeFile})`;
    ...
    console.log(event.target.files.length, "files added. Total:", files.length);
    input.value = '';
});
