...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.PromiseSafeBufferCache = void 0;
...
class PromiseSafeBufferCache {
    constructor() {
        this._cache = new Map();
        this._cacheMutex = new mutex_1.Mutex();
        ...
        for (let i = 1; i < sortedEntries.length; i++) {
            ...
            if (currentOffset + currentBuffer.length === offset) {
                ...
                if (bufferToMerge.length)
                    currentBuffer = polyfill_fs_1.Buffer.concat([currentBuffer, bufferToMerge]);
            }
            else {
    ...
    async get(offset, length) {
        ...
        const entry = Array.from(this._cache.entries()).find(([cacheOffset, buffer]) => {
...
exports.PromiseSafeBufferCache = PromiseSafeBufferCache;
...
const { EventEmitter } = (0, polyfill_1.polyfill)({
    EventEmitter: _EventEmitter
}, "events");
const out = EventEmitter;
exports.EventEmitter = out;
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.fs = exports.Buffer = void 0;
const polyfill_1 = __webpack_require__(/*! ./polyfill */ "./src/polyfill.ts");
const buffer_1 = __webpack_require__(/*! buffer */ "./node_modules/buffer/index.js");
const { Buffer: PolyfillBuffer } = (0, polyfill_1.polyfill)({ Buffer: buffer_1.Buffer }, "node:buffer");
...
const { read, open, close, statSync, existsSync } = (0, polyfill_1.polyfill)({
    read: function (file, buffer, offset, length, position, callback) {
        ...
        assert("buffer", buffer, [exports.Buffer]);
        ...
        file.arrayBuffer().then((blobBuffer) => {
            blobBuffer = exports.Buffer.from(blobBuffer);
...
const polyfill_1 = __webpack_require__(/*! ./polyfill */ "./src/polyfill.ts"); // see polyfill.ts for usage
const { 
// path
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.dbpf = exports.load = exports.DBPFEntry = exports.DBPF = void 0;
const polyfill_1 = __webpack_require__(/*! ./polyfill */ "./src/polyfill.ts");
const polyfill_fs_1 = __webpack_require__(/*! ./polyfill.fs */ "./src/polyfill.fs.ts");
...
const buffercache_1 = __webpack_require__(/*! ./buffercache */ "./src/buffercache.ts");
let { resolve } = (0, polyfill_1.polyfill)({ resolve: (...paths) => paths.join("/") }, "node:path");
const { assign: obj_assign, getOwnPropertyDescriptor: obj_getOwnPropertyDescriptor, } = Object;
const HEADERLENGTH = 0x60; // 96 bytes
const ENTRYLENGTH = 0x20; // 32 bytes
function MagicByte(string) {
...
class DBPF extends polyfill_events_1.EventEmitter {
    ...
    static { this.magic = MagicByte("DBPF"); }
    get file() {
        return this._file;
    }
    get filepath() {
    ...
    _parseHeader(buffer, callback) {
        ...
        const reader = new BufferReader_1.BufferReader(headerbuffer);
    ...
    constructor(file) {
        super();
        this.magic = DBPF.magic; // for inheritance, in case I ever write a dbbf reader
        this._file = "";
        this._filepath = "";
        this._filename = "";
        this._extension = "";
        ...
        this._cache = new buffercache_1.PromiseSafeBufferCache();
        if (typeof file === "string")
            file = file.trim();
        if (file instanceof File || (typeof file === "string" && file.length && polyfill_1.polyfill.isNode)) {
            this.file = file;
        }
        else
    ...
    read(length = this._filesize, offset = 0, cache = true, callback) {
        this.validate(callback);
        const out_buffer = polyfill_fs_1.Buffer.alloc(length);
        const out = new Promise(async (out_resolve, out_reject) => {
            const emit_reject = (error) => {
                this.emit("error", error);
                if (typeof callback === "function")
                    callback(error);
                out_reject(error);
            ...
            if (this._file instanceof File) {
                const blobRead_promise = new Promise((blobRead_resolve, blobRead_reject) => {
                    polyfill_fs_1.fs.read(this._file, out_buffer, 0, length, offset, async (err, bytesRead, buffer) => {
                        if (err) {
                ...
                const open_promise = new Promise((open_resolve, open_reject) => {
                    polyfill_fs_1.fs.open(this._filepath, 'r', (err, fd) => {
                        if (err) {
                ...
                open_promise
                    .then((fd) => {
                    const read_promise = new Promise((read_resolve, read_reject) => {
                        polyfill_fs_1.fs.read(fd, out_buffer, 0, length, offset, async (err, bytesRead, buffer) => {
                            if (err) {
                            ...
                            const close_promise = new Promise((close_resolve, close_reject) => {
                                polyfill_fs_1.fs.close(fd, (err) => {
                                    if (err) {
    ...
    init(callback) {
        this.validate(callback);
        const out = this._initializer ? this._initializer : new Promise((out_resolve, out_reject) => {
            this.read(HEADERLENGTH, 0, true, callback)
                .then((buffer) => {
                this._parseHeader(buffer, callback)
                    .then(() => {
                    this.emit("init", this);
                    out_resolve(this);
                }).catch(out_reject);
    ...
    loadAll(callback) {
        this.validate(callback);
        const out = new Promise((out_resolve, out_reject) => {
            this.read(this.filesize, 0, true, callback)
                .then((buffer) => {
...
exports.DBPF = DBPF;
class DBPFEntry {
    constructor(file, type, // FourBytes
    group, // FourBytes
    instance, offset, // FourBytes
    size, unknown) {
        this.file = file;
        this.type = type;
        this.group = group;
...
class DBPFIndexTable extends polyfill_events_1.EventEmitter {
    init() {
        return this._ready;
    }
    constructor(file) {
        ...
        this._ready = new Promise((ready_resolve, ready_reject) => {
            file.read(table_size - 4, table_offset + 4).then((buffer) => {
                this._buffer = buffer;
                ready_resolve();
            }).catch(ready_reject);
        }).then(() => {
        ...
        const handler = {
            ...
            getOwnPropertyDescriptor: function (target, prop) {
                if (entries.has(Number(prop)))
                    ...
                    return obj_assign({}, obj_getOwnPropertyDescriptor(self, prop), { writable: false, configurable: true });
            },
    ...
    get(index) {
        ...
        const reader = new BufferReader_1.BufferReader(this._instance._buffer);
        reader.move(index * ENTRYLENGTH);
        const type = reader.getInt(); // 0 -> 4 bytes
        const group = reader.getInt(); // 4 -> 8 bytes
        const instance_high = reader.getInt(); // 8 -> 12 bytes
        const instance_low = reader.getInt(); // 12 -> 16 bytes
        const offset = reader.getInt(); // 16 -> 20 bytes
        const size_file = reader.getInt(); // 20 -> 24 bytes
        const size_memory = reader.getInt(); // 24 -> 28 bytes
        const size_compressed = reader.getShort(); // 28 -> 30 bytes
        const unknown = reader.getShort(); // 30 -> 32 bytes
        ...
        const base_entry = new DBPFEntry(this._instance.file, type, group, instance, offset, {
            file: size_file,
            memory: size_memory,
            ...
            for (const plugin of plugins) {
                ...
                for (const key in filters) {
                    if (key in modified_entry) {
                        if (modified_entry[key] !== filters[key]) {
                            modified_entry = plugin.module(modified_entry);
                            break;
                        }
                    }
                }
...
class Plugins extends serde_1.Deserialized {
    static from(obj) {
        const self = this instanceof Plugins ? this : new Plugins(undefined);
    ...
    constructor(filepath) {
        ...
        for (const key in this) {
            if (typeof key !== "number")
                ...
                if (polyfill_1.polyfill.isNode) {
                    ...
                    if (!polyfill_fs_1.fs.existsSync(path)) {
                        throw new Error(`Plugin-Deserializer: Plugin not found: ${path}`);
                    }
                    this[key].path = path;
                }
                this[key].module = polyfill_1.polyfill.require(this[key].path);
        ...
        const handler = {
            ...
            ownKeys: function (target) {
                return Array.from(self._plugins.keys());
            }
        };
        this; // to generate compiler errors for missing methods
        return this._proxy = new Proxy(this, handler);
    }
    get length() {
    ...
    reduce(callback, initialValue) {
        return this._instance._plugins.reduce(callback, initialValue);
    }
    reduceRight(callback, initialValue) {
        return this._instance._plugins.reduceRight(callback, initialValue);
    }
...
class Plugin {
    constructor(filters, path) {
        ...
        if (polyfill_1.polyfill.isNode) {
            ...
            if (!polyfill_fs_1.fs.existsSync(resolved)) {
        ...
        this.module = polyfill_1.polyfill.require(this.path);
    }
}
function load(pluginsFile) {
    return DBPFIndexTable.plugins = Plugins.read(pluginsFile);
}
exports.load = load;
// umd export
exports.dbpf = {
